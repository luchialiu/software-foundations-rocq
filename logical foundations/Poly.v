...
(* using fold to compute the length of a list of booleans,
   where X = bool and Y = nat
...
*)
(* We prove that for all X, l, n, if length l = n then nth_error l n = None.

Proof by induction on l.

Base case: l = [].
Then length l = 0, so n = 0.
nth_error [] 0 = None, so the statement holds.

Inductive case: l = a :: l'.
Assume the statement holds for l' (induction hypothesis):
for all n, if length l' = n then nth_error l' n = None.

Now suppose length (a :: l') = n.
Then n = S (length l').

We must show that nth_error (a :: l') n = None.

By definition,
nth_error (a :: l') n = nth_error l' (pred n)

Since n = S (length l'), we have pred n = length l'

So, nth_error l' (length l') = None,
