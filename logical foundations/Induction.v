...
(** **** Exercise: 2 stars, advanced, optional (add_comm_informal)
    ...
    By the definition of [+], this follows from S (n' + m) = m + S n'.
    Rewrite using plus_n_Sm, this follows from S (n' + m) = S (m + n')，
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    Proof: (* By induction on[n]
    - First, suppose n = 0. We must show that (0 =? 0) = true.
      By the definition of =?, comparing 0 with 0 returns true. 
      So the claim holds in this case.
      
    - Next, suppose n = Sn', where as induction hypothesis we assume 
      (n' =? n') = true. 
      We must show that (Sn' = Sn') = true. By the definition of =?,
      when both arguments are successors, it reduces the comparison 
      to their predecessors; that is, (Sn' =? Sn') evaluates to 
      (n' =? n'). By the induction hypothesis, (n' =? n') = true.
      Therefore (Sn' =? Sn') = true, which completes the inductive step.
      
    By induction for all n, (n =? n) = true. Qed.*)
*)
...
Proof.
  ...
  - replace (m + (n + p)) with ((m + n) + p).
    + rewrite (add_comm n m). reflexivity.
...
(* helper lemma: multiplication on the right by a successor*)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IHn. simpl. rewrite add_comm. rewrite plus_n_Sm. simpl.
...
(* Because the inductive definition of bin allows for multiple representations
   of the same number. For example, Z, B0 Z, and B0 (B0 Z) can all represent
   zero. Converting a bin to nat and back to bin does not necessarily return 
   the original structure. The function nat_to_bin builds a canonical 
   representation by repeated incr, so it eliminates extra zeros and may produce
   a different but numerically equivalent bin.*)
