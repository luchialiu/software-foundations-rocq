...
(* apply is used when we have a theorem or hypothesis whose conclusion directly mathes the goal.
This allows us to reduce to the goal to its premises (backward reasoning).
rewrite uses an equality to transform part of the goal into an equivalent form
so it becomes solvable. Both can be used with equalities, but apply works 
when the goal exactly matches the equality's direction, while rewrite is used
when we need to change the goal's structure.
...
Proof.
  ...
  injection H1 as Hxz Hyj.
  rewrite H2 in Hyj.
  injection Hyj as Hyz.
  rewrite Hxz.
  rewrite Hyz.
  reflexivity.
Qed.
...
(* Take arbitrary natural numbers n and m, and suppose n =? m = true. 
We prove that n = m by induction on n, keeping m arbitrary. 
In the base case, n = 0. If m = 0, then clearly n = m. 
If m = S m', then 0 =? S m' simplifies to false, contradicting the assumption that it equals true. 
For the inductive step, assume n = S n' 
and assume as induction hypothesis that for every natural number m, 
if n' =? m = true, then n' = m. 
Now let m be arbitrary and suppose S n' =? m = true. 
If m = 0, then S n' =? 0 simplifies to false, again a contradiction. 
If m = S m', then S n' =? S m' simplifies to n' =? m', 
so from the assumption we get n' =? m' = true. 
By the induction hypothesis, n' = m'. 
Therefore S n' = S m', that is, n = m.  *)
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: *)
   (* We prove that for all natural numbers n and m, (n =? m) = (m =? n) by induction on n.
In the base case n = 0, we consider m. 
If m = 0, both sides evaluate to true. 
If m = S m', both sides evaluate to false.
For the inductive step, assume the statement holds for n'. 
For n = S n', we consider m. If m = 0, both sides evaluate to false. 
If m = S m', both sides reduce to (n' =? m') and (m' =? n'), 
which are equal by the induction hypothesis. 
Therefore, the statement holds for all n and m.
