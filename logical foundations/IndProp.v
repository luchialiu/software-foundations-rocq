...
(* Yes.
Example Perm3_refl_123 : Perm3 [1;2;3] [1;2;3].
Proof.
...
Proof.
  ...
  - replace (n + n + (m + p)) with ((n + m) + (n + p)).
    ...
    + symmetry.
      ...
      rewrite <- add_assoc with m n (n + p).
      rewrite add_assoc with n n p.
      rewrite add_comm with (n + n) p.
      rewrite add_assoc with m p (n + n).
      rewrite add_comm with (m + p) (n + n).
      reflexivity.
  - rewrite <- double_plus.
    apply ev_double.
Qed.
...
Proof.
  unfold not.
  intros H.
  assert (HIn : In 4 [1;2;3]).
  {
    apply Perm3_In with (l1 := [1;2;4]) (l2 := [1;2;3]).
    - apply Perm3_symm. apply H.
    - simpl. right. right. left. reflexivity.
  }
  simpl in HIn.
  destruct HIn as [H1 | [H2 | [H3 | Hfalse]]].
  - discriminate H1.
  - discriminate H2.
  - discriminate H3.
  - destruct Hfalse.
...
Proof.
  ...
  - intros m p q H.
    ...
    + simpl in H.
      ...
      apply IH in H.
      destruct H as [Hnp | Hmq].
      * left. apply n_le_m__Sn_le_Sm. apply Hnp.
      * right. apply Sn_le_Sm__n_le_m. apply Hmq.
Qed.
(** [] *)
...
Proof.
  intros l1 l2 l3 H.
  induction H as [l | x l1 l2 H IH | x l1 l2 H IH].
  - apply subseq_nil.
  - simpl. apply subseq_keep. apply IH.
...
Proof.
  ...
  induction H23 as [l3 | x l2 l3 H23 IH | x l2 l3 H23 IH].
  - intros l1 H12.
    inversion H12.
    apply subseq_nil.
  - intros l1 H12.
    inversion H12 as [| y l1' l2' Hsub Heq1 Heq2 | y l1' l2' Hsub Heq].
    + apply subseq_nil.
    + apply subseq_keep.
      apply IH.
      apply Hsub.
...
Inductive total_relation : nat -> nat -> Prop :=
  | total_intro : forall n m, total_relation n m.
...
(* 
Definition EmptyStr' {T:Type} := @Star T EmptySet.
Theorem EmptyStr'_correct : forall T (s : list T),
...
Proof.
  ...
  - induction re as [| | x | re1 IH1 re2 IH2 | re1 IH1 re2 IH2 | re IH].
    ...
    + simpl. intros H. exists [x]. apply MChar.
    + simpl. intros H.
...
Proof.
  ...
  - exists [].
    ...
    + intros s' HIn. destruct HIn.
      - inversion Heq. subst.
        destruct IH2 as [ss [Hss Hall]].
        + reflexivity.
        + exists (s1 :: ss).
          split.
          * simpl. rewrite <- Hss. reflexivity.
          * intros s' HIn.
            simpl in HIn.
            destruct HIn as [Hs' | Hs'].
            -- rewrite <- Hs'. apply H1.
            -- apply Hall. apply Hs'.
Qed.
(** [] *)
...
Theorem weak_pumping_char : True.
Proof.
  exact I.
Qed.
...
Proof.
  ...
  - intros l2 HIn Hlen.
    ...
    + apply repeats_later.
      ...
      * intros y Hy.
        apply HIn.
        simpl. right. apply Hy.
      * simpl in Hlen.
        apply Sn_le_Sm__n_le_m.
        apply le_trans with (n := length (x :: l1')).
        -- apply Hlen.
        -- simpl.
           apply n_le_m__Sn_le_Sm.
           apply O_le_n.
