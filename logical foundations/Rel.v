...
(** **** Exercise: 2 stars, standard, optional (le_Sn_n_inf)
    ...
    Proof: *)
    (* Assume for contradiction that S n ≤ n.
The ≤ relation means you can reach n from S n by repeatedly applying le_S. But le_S only increases the right-hand side, never decreases it. So starting from S n, you can only reach numbers ≥ S n.
But n < S n, so n is strictly smaller than S n. Therefore it is impossible for S n ≤ n.
Contradiction.
