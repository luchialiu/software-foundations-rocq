...
Proof.
  intros n m Hn Hm.
  (*two premises, so no need for brackets*)
  rewrite Hn. rewrite Hm.
  reflexivity.
Qed.
...
(*can also write:
  ...
  destruct absurd.*)
Qed.
  (* contra has zero constructors, so it's vacuously true - proof ends.*)
...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P HNP Q contra. (* must write in the order of how things are introduced in the theorem, or it can't be interpreted properly*)
  unfold not in HNP.
  apply HNP in contra.
  destruct contra.
Qed.
...
(* Proof. Suppose P is true. Negating P then leads to a contradiction. Therefore, negated negated P must be true. Qed.*)
...
Proof.
  ...
  (*unfold not in HNQ. - doesn't do a thing here; exact proof depends on intros*)
  apply HPQ in HP.
  apply HNQ in HP.
  destruct HP.
Qed.
...
(* Proof. Suppose P is true. It is a contradiction to have both P and not P, thus proved. Qed. *)
...
Definition nil_cons_disc {X : Type} (l : list X) : Prop :=
  match l with
  | nil => True
  | _ :: _ => False
  end.
...
Proof.
  intros X x xs H.
  assert (Hnil : nil_cons_disc (@nil X) ).
  { simpl. apply I. }
  rewrite H in Hnil.
  simpl in Hnil.
  apply Hnil.
...
Proof.
  unfold not.
  intros X P Hforall Hexists.
  destruct Hexists as [x Hnot].
  apply Hnot.
  apply Hforall.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H.
    ...
    + simpl. split.
      ...
      * apply IH. intros y Hy.
        apply H. simpl. right. apply Hy.
  - induction l as [|x xs IH].
    + intros H y Hy. simpl in Hy. destruct Hy.
    + intros H y Hy.
      simpl in H. destruct H as [HPx Hall].
      simpl in Hy. destruct Hy as [Hxy | Hy].
      * rewrite <- Hxy. apply HPx.
      * apply IH.
        -- apply Hall.
        -- apply Hy.
Qed.
...
Proof.
  ...
  - intros l2. destruct l2 as [|x2 xs2].
    + simpl. split.
  ...
  - intros l2. destruct l2 as [|x2 xs2].
    ...
    + simpl. split.
      * intros H.
        ...
        apply Heqb in Hhead.
        apply IH in Htail.
        rewrite Hhead. rewrite Htail.
        reflexivity.
      * intros H.
        injection H as Hhead Htail.
        apply andb_true_iff.
        split.
        -- apply Heqb. apply Hhead.
...
(* This specification says exactly when forallb returns true, but it does
   not directly describe computational behavior such as short-circuiting.
   For example, it does not say that forallb stops checking the rest of
   the list as soon as test x returns false. It also does not characterize
   how many times test is called. The theorem captures the logical meaning
   of forallb, not its operational cost/behavior.
