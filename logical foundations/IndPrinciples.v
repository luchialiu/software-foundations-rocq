...
Proof.
  ...
  intros P Hempty Hleaf Hbranch t.
  induction t as [| b | b t1 IH1 t2 IH2].
  - exact Hempty.
  - apply Hleaf.
  - apply Hbranch.
    + exact IH1.
...
Theorem add_comm'' : forall n, P_comm n.
Proof.
  apply nat_ind.
...
Definition better_t_tree_ind_type : Prop:=
  forall (X : Type) (P : t_tree X -> Prop),
    P t_leaf ->
    (forall l v r, P l -> P r -> P (t_branch (l, v, r))) ->
    forall t, P t.
...
Definition better_t_tree_ind : better_t_tree_ind_type:=
  fun (X : Type) (P : t_tree X -> Prop)
      (Hleaf : P t_leaf)
      (Hbranch : forall l v r,
          P l -> P r -> P (t_branch (l, v, r))) =>
    fix F (t : t_tree X) : P t :=
      match t with
      | t_leaf => Hleaf
      | t_branch ((l, v), r) =>
          Hbranch l v r (F l) (F r)
      end.
